document.addEventListener("DOMContentLoaded", () => {
  ...
  const add_tokenSizeMultiplier = document.getElementById(
    "tokenSizeMultiplier"
  );
  const selectedTokenView = document.getElementById("main-view-selected-token");
  const edit_tokenLetterPreview = document.getElementById(
    "editTokenLetterPreview"
  );
  ...
  const edit_tokenImagePreview = document.getElementById(
  ...
  const edit_tokenSizeMultiplier = document.getElementById(
    "editTokenSizeMultiplier"
  );
  ...
  const healthDisplayContainer = document.getElementById(
  ...
  const abilityDescriptionInput = document.getElementById(
  ...
  const inventoryItemDescription = document.getElementById(
  ...
  const inventoryItemPlaceholder = document.getElementById(
    "inventoryItemPlaceholder"
  );
  const inventoryItemIsMagical = document.getElementById(
    "inventoryItemIsMagical"
  );
  const confirmInventoryItemBtn = document.getElementById(
    "confirmInventoryItemBtn"
  );
  const cancelInventoryItemBtn = document.getElementById(
    "cancelInventoryItemBtn"
  );
  ...
  const noSavedCharactersMessage = document.getElementById(
    "noSavedCharactersMessage"
  );
  ...
  const characterLibraryActions = document.getElementById(
    "characterLibraryActions"
  );
  const addTransparentBgCheckbox = document.getElementById(
    "addTransparentBgCheckbox"
  );
  ...
  document
    .querySelectorAll('#deathSavesTracker input[type="checkbox"]')
    .forEach((checkbox) => {
      checkbox.addEventListener("change", updateSelectedToken);
    });

  addTransparentBgCheckbox.addEventListener("change", () => {
    document.getElementById("tokenColor").disabled =
      addTransparentBgCheckbox.checked;
  });
  ...
  function showCustomModal({
    ...
    return new Promise((resolve) => {
      ...
      if (type === "confirm") {
        ...
        // para 'info', 'success', 'warning', 'error'
  ...
  channel.onmessage = (event) => {
    ...
    switch (type) {
      case "EVENT_TOKEN_MOVED":
        ...
        if (movedToken) {
          movedToken.position.x = payload.x;
          movedToken.position.y = payload.y;
          if (visionModeActive)
            broadcast("CMD_DRAW_VISION", { tokens, walls, cellSize });
        }
        isSceneDirty = true;
        break;
      case "EVENT_TOKEN_CLICKED":
        selectToken(payload.id);
        break;
      case "EVENT_MAP_CLICKED":
        deselectToken();
        break;
      ...
      case "EVENT_GRID_ALIGNED":
        ...
        break;
      case "EVENT_FOG_PAINTED":
        checkEnemyDiscovery(payload.visibleEnemies);
        break;
  ...
  openPlayerViewBtn.addEventListener("click", () => {
    ...
    setTimeout(() => {
      if (isMapLoaded) {
        ...
        broadcast("CMD_LOAD_NEW_MAP", { src: mapSrc });
        setTimeout(() => {
  ...
  brushSizeInput.addEventListener("input", (e) => {
    ...
    broadcast("CMD_SET_BRUSH_SIZE", { size: brushSize });
  });
  ...
  healthModifierBtns.forEach((btn) =>
    btn.addEventListener("click", () =>
      applyHealthChange(parseInt(btn.dataset.amount))
    )
  );
  ...
  aoeParamsContainer
    ...
    .forEach((input) => {
      input.addEventListener("input", () => {
        if (activeAoeType) broadcastAoeState();
      });
    });
  ["str", "dex", "con", "int", "wis", "car"].forEach((stat) => {
    document
      .getElementById(`add_${stat}_score`)
      .addEventListener("input", () => updateCharStatUI("add", stat));
    document
      .getElementById(`add_${stat}_save_prof`)
      .addEventListener("change", () => updateSavingThrowsUI("add"));
  });
  document
    .getElementById("add_proficiency_bonus")
    .addEventListener("input", () => updateSavingThrowsUI("add"));
  ["str", "dex", "con", "int", "wis", "car"].forEach((stat) => {
    document
      .getElementById(`edit_${stat}_score`)
      .addEventListener("input", () => {
        updateCharStatUI("edit", stat);
        updateSelectedToken();
      });
    document
      .getElementById(`edit_${stat}_save_prof`)
      .addEventListener("change", () => {
        updateSavingThrowsUI("edit");
        updateSelectedToken();
      });
  });
  document
    .getElementById("edit_proficiency_bonus")
    .addEventListener("input", () => {
      updateSavingThrowsUI("edit");
      updateSelectedToken();
    });
  document
    .getElementById("edit_armor_class")
    .addEventListener("input", () => updateSelectedToken());
  document
    .getElementById("edit_speed")
    .addEventListener("input", () => updateSelectedToken());
      ...
      if (audio.loop) {
        if (activeLoopingSound === audio) {
          ...
          if (activeLoopingSound) {
            ...
            document
              .querySelector(".sound-btn.active")
  ...
  tokenHeaderInfo.addEventListener("click", (e) => {
    if (tokenInfoEdit.style.display === "none") {
      tokenInfoView.style.display = "none";
      tokenInfoEdit.style.display = "block";
  ...
  const saveHeaderChanges = () => {
    ...
    const token = tokens.find((t) => t.id === selectedTokenId);
    if (!token) return;
    token.identity.name = document.getElementById("editTokenName").value;
    token.identity.char_class = document.getElementById("editTokenClass").value;
    token.identity.level =
      parseInt(document.getElementById("editTokenLevel").value) || 1;
    token.identity.race = document.getElementById("editTokenRace").value;
    updateTokenInUI(token);
    updateTokenList();
    broadcast("CMD_UPDATE_TOKEN_DATA", { tokenData: token });
  ...
  function updateTokenInUI(token) {
    ...
    document
      .querySelectorAll("#tokenList li.selected-in-list")
      .forEach((li) => li.classList.remove("selected-in-list"));
    ...
    document.getElementById("viewTokenClass").textContent =
      token.identity.char_class || "Clase";
    document.getElementById("viewTokenLevel").textContent =
      token.identity.level || 1;
    document.getElementById("viewTokenRace").textContent =
      token.identity.race || "Raza";
    ...
    if (token.identity.image) {
      ...
      document.getElementById("editTokenLetterPreview").textContent =
        token.identity.letter;
      document.getElementById("editTokenLetterPreview").style.backgroundColor =
        token.appearance.color;
    }

    // --- PESTAÑA COMBATE ---
    document.getElementById("edit_proficiency_bonus").value =
      token.stats.proficiencyBonus;
    document.getElementById("editTokenTurn").value =
    ...
    document.getElementById("editTokenVision").value =
      token.stats.vision.radius;
    document.getElementById("editTokenHealthMax").value =
      token.stats.health.max;
    healthDisplay.textContent = token.stats.health.current;
    healthDisplay.className = `health-display ${getHealthColorClass(
      token.stats.health.current,
      token.stats.health.max
    )}`;
    renderTokenStatesEditor(token);

    // --- INICIO: LÓGICA CORREGIDA PARA CAMPOS DE JEFE ---
    const editBossToggleContainer = document.getElementById(
      "editBossToggleContainer"
    );
    const editBossToggle = document.getElementById("editBossToggle");
    const editPresentationContainer = document.getElementById(
      "editPresentationContainer"
    );
    const notesTabPresentationContainer = document.getElementById(
      "editPresentationContainer"
    ); // Referencia al de la pestaña Notas
    ...
    if (token.identity.type === "enemy") {
      ...
      if (isBoss) {
        document.getElementById("editTokenPresentation").value =
    ...
    if (token.identity.type === "player") {
      ...
      const saves = token.info.deathSaves || {
        successes: [false, false, false],
        failures: [false, false, false],
      };
      saves.successes.forEach((val, i) => {
        document.getElementById(`d-save-s${i + 1}`).checked = val;
      });
      saves.failures.forEach((val, i) => {
        document.getElementById(`d-save-f${i + 1}`).checked = val;
      });
    } else {
    ...
    if (token.identity.type === "enemy") {
      ...
      defeatedBtn.textContent = token.isDefeated
        ? "Reanimar Enemigo"
        : "Abatir Enemigo";
    } else {
    ...
    ["str", "dex", "con", "int", "wis", "car"].forEach((stat) => {
      if (token.stats.characteristics && token.stats.characteristics[stat]) {
        document.getElementById(`edit_${stat}_score`).value =
          token.stats.characteristics[stat].score;
        document.getElementById(`edit_${stat}_save_prof`).checked =
          token.stats.characteristics[stat].proficient;
      }
      updateCharStatUI("edit", stat);
    });
    document.getElementById("editTokenColor").value =
      token.appearance.color === "transparent"
        ? "#000000"
        : token.appearance.color;
    document.getElementById("editTokenBorderColor").value =
      token.appearance.borderColor || "#000000";
    editBorderCheckbox.checked = !!token.appearance.borderColor;
    const editTransparentBgCheckbox = document.getElementById(
      "editTransparentBgCheckbox"
    );
    editTransparentBgCheckbox.checked =
      token.appearance.color === "transparent";
    document.getElementById("editTokenColor").disabled =
      editTransparentBgCheckbox.checked;
    editTransparentBgCheckbox.onchange = () => {
      document.getElementById("editTokenColor").disabled =
        editTransparentBgCheckbox.checked;
      updateSelectedToken();
    };
    document.getElementById("editTokenSizeMultiplier").value =
      token.position.sizeMultiplier || 1;
  ...
  function saveCurrentScene() {
    ...
    if (!playerViewWindow || playerViewWindow.closed) {
      showCustomModal({
        title: "Atención",
        message:
          "La ventana del jugador debe estar abierta para guardar el estado completo de la escena (incluida la niebla de guerra)..",
        type: "warning",
      });
      return;
    }
  ...
  async function finalizeSceneSave(sceneName, sceneId, fogDataUrl) {
    try {
      ...
      const tokenPromises = tokens.map(async (t, index) => {
        ...
        if (cleanToken.identity.image)
          cleanToken.identity.imageAssetId = imageAssetId;
  ...
  async function loadSceneById(sceneId) {
    try {
      ...
      const fogAsset = sceneMetadata.fogAssetId
        ? await assetStore.get(sceneMetadata.fogAssetId)
        : null;
      ...
      const tokenAssetPromises = sceneMetadata.tokens.map((tokenMeta) => {
        const assetId =
          tokenMeta.identity?.imageAssetId || tokenMeta.imageAssetId;
      ...
      tokens = sceneMetadata.tokens.map((tokenData, index) => {
        ...
        if (
          migratedTokenData.info &&
          Array.isArray(migratedTokenData.info.inventory)
        ) {
          migratedTokenData.info.inventory =
            migratedTokenData.info.inventory.map(migrateInventoryItem);
        }
  ...
  async function renderSavedScenesList() {
    ...
    const validMapKeys = scenes
      .map((scene) => scene.mapAssetId)
      .filter((key) => key);
    ...
    if (validMapKeys.length > 0) {
      ...
      const assets = await Promise.all(
        validMapKeys.map((key) => store.get(key))
      );
  ...
  async function deleteSceneById(sceneId) {
    const confirmed = await showCustomModal({
      ...
      message:
        "¿Estás realmente seguro de que quieres eliminar esta escena? Esta acción es irreversible.",
      type: "confirm",
    ...
    if (!sceneToDelete) {
      console.warn(
        "Se intentó borrar una escena que ya no existe:",
        numericSceneId
      );
  ...
  function migrateTokenData(oldToken) {
    if (
      oldToken.stats &&
      oldToken.stats.characteristics &&
      oldToken.info &&
      oldToken.info.abilities
    ) {
      // Si el token ya tiene la estructura moderna, solo nos aseguramos de que los nuevos campos existan
      if (!oldToken.identity.hasOwnProperty("isBoss"))
        oldToken.identity.isBoss = false;
      if (!oldToken.info.hasOwnProperty("presentation"))
        oldToken.info.presentation = "";
      return oldToken;
    }
    ...
    return {
      ...
      stats: {
        ...
        vision: {
          radius: oldToken.stats?.vision?.radius ?? oldToken.visionRadius ?? 0,
        },
        health: {
          current:
            oldToken.stats?.health?.current ?? oldToken.health_current ?? 0,
        ...
        characteristics: oldToken.stats?.characteristics || {
          ...
          dex: { score: 10, proficient: false },
          con: { score: 10, proficient: false },
          int: { score: 10, proficient: false },
          wis: { score: 10, proficient: false },
          car: { score: 10, proficient: false },
        },
      },
      ...
      info: {
        ...
        deathSaves: existingInfo.deathSaves || {
          successes: [false, false, false],
          failures: [false, false, false],
        },
  ...
  function handleImageUpload(event) {
    ...
    reader.onload = (e) => {
      ...
      document.getElementById("mapImageInput")._dataUrl = src;
      loadNewMap(src);
    };
    reader.readAsDataURL(file);
  }
  function loadNewMap(src) {
    ...
    selectedTokenId = null;
    visionModeActive = false;
    toggleVisionBtn.textContent = "Iniciar Visión";
    updateTokenList();
    updateDoorList();
    broadcast("CMD_LOAD_NEW_MAP", { src });
    broadcastGridSettings();
  }
  ...
  async function addToken(existingCharacter = null, fromLibrary = false) {
    let tokenData;
    if (fromLibrary && existingCharacter) {
      tokenData = existingCharacter;
    } else {
      ...
      ["str", "dex", "con", "int", "wis", "car"].forEach((stat) => {
        characteristics[stat] = {
          score:
            parseInt(document.getElementById(`add_${stat}_score`).value) || 10,
      ...
      tokenData = {
        id: Date.now(),
        isDiscovered:
          document.querySelector('input[name="add_tokenType"]:checked')
            .value === "player",
        identity: {
          name,
          type: document.querySelector('input[name="add_tokenType"]:checked')
            .value,
          letter,
          image: imageBase64,
          char_class:
            document.getElementById("tokenClass").value || "Aventurero",
        ...
        stats: {
          ...
          proficiencyBonus:
            parseInt(document.getElementById("add_proficiency_bonus").value) ||
            2,
          speed: parseInt(document.getElementById("add_speed").value) || 30,
          armorClass:
            parseInt(document.getElementById("add_armor_class").value) || 10,
          characteristics,
        },
        appearance: {
          color: document.getElementById("tokenColor").value,
          borderColor: document.getElementById("addBorderCheckbox").checked
            ? document.getElementById("tokenBorderColor").value
            : null,
        },
        info: {
  ...
  function deleteToken(tokenId) {
    ...
    updateTokenList();
    broadcast("CMD_DELETE_TOKEN", { id: tokenId });
    if (visionModeActive)
      broadcast("CMD_DRAW_VISION", { tokens, walls, cellSize });
  }
  function selectToken(tokenId) {
    ...
    const tokenViewButton = mainColumn.querySelector(
      '[data-view="main-view-selected-token"]'
    );
    if (tokenViewButton) {
  ...
  function deselectToken(isSwitching = false) {
    ...
    const oldListItem = tokenListUl.querySelector(
      `li[data-id="${selectedTokenId}"]`
    );
  ...
  function updateSelectedToken() {
    ...
    token.identity.char_class =
      document.getElementById("editTokenClass").value.trim() || "Clase";
    token.identity.level =
      parseInt(document.getElementById("editTokenLevel").value) || 1;
    token.identity.race =
      document.getElementById("editTokenRace").value.trim() || "Raza";

    // --- INICIO: GUARDADO DE NUEVOS CAMPOS ---
    token.identity.isBoss = document.getElementById("editBossToggle").checked;
    token.info.presentation = document.getElementById(
      "editTokenPresentation"
    ).value;
    // --- FIN: GUARDADO DE NUEVOS CAMPOS ---

    token.stats.initiative =
      parseInt(document.getElementById("editTokenTurn").value) || 0;
    token.stats.vision.radius =
      parseInt(document.getElementById("editTokenVision").value) || 0;
    token.stats.health.max =
      parseInt(document.getElementById("editTokenHealthMax").value) || 0;
    token.stats.proficiencyBonus =
      parseInt(document.getElementById("edit_proficiency_bonus").value) || 0;
    token.stats.speed =
      parseInt(document.getElementById("edit_speed").value) || 0;
    token.stats.armorClass =
      parseInt(document.getElementById("edit_armor_class").value) || 0;

    ["str", "dex", "con", "int", "wis", "car"].forEach((stat) => {
      token.stats.characteristics[stat].score =
        parseInt(document.getElementById(`edit_${stat}_score`).value) || 10;
      token.stats.characteristics[stat].proficient = document.getElementById(
        `edit_${stat}_save_prof`
      ).checked;
    });

    token.appearance.color = document.getElementById(
      "editTransparentBgCheckbox"
    ).checked
      ? "transparent"
      : document.getElementById("editTokenColor").value;
    token.appearance.borderColor = editBorderCheckbox.checked
      ? document.getElementById("editTokenBorderColor").value
      : null;
    token.info.notes = document.getElementById("editTokenNotes").value;
    token.position.sizeMultiplier =
      parseFloat(document.getElementById("editTokenSizeMultiplier").value) || 1;
  ...
  function toggleVisionMode() {
    ...
    toggleVisionBtn.textContent = visionModeActive
      ? "Detener Visión"
      : "Iniciar Visión";
    broadcast("CMD_SET_VISION_MODE", {
  ...
  function resetFog() {
    if (!confirm("¿Reiniciar toda la niebla de guerra?")) return;
    tokens.forEach((t) => {
      if (t.identity.type === "enemy") t.isDiscovered = false;
    });
    broadcast("CMD_RESET_FOG");
    if (visionModeActive)
      broadcast("CMD_DRAW_VISION", { tokens, walls, cellSize });
    updateTokenList();
  }
  ...
  function handleBrushModeChange(event) {
    ...
    const otherCheckbox =
      checkbox.id === "brushReveal" ? brushHideCheckbox : brushRevealCheckbox;
    if (checkbox.checked) {
    ...
    broadcast("CMD_SET_BRUSH_MODE", { mode: activeBrushMode });
  }

  function toggleAlignGridMode(forceState) {
    isAligningGrid =
      typeof forceState === "boolean" ? forceState : !isAligningGrid;
    alignGridModeBtn.classList.toggle("active", isAligningGrid);
    alignGridModeBtn.textContent = isAligningGrid
      ? "Cancelar Alineación"
      : "Activar Modo Alineación";
    broadcast("CMD_SET_GRID_ALIGN_MODE", { active: isAligningGrid });
  }
  function broadcastGridSettings() {
    broadcast("CMD_SET_GRID_SETTINGS", {
      gridSettings: {
        visible: gridVisible,
        color: gridColor,
        opacity: gridOpacity,
  ...
  const updateSavingThrowsUI = (prefix) => {
    ...
    const profBonus =
      parseInt(document.getElementById(`${prefix}_proficiency_bonus`).value) ||
      0;
    stats.forEach((stat) => {
      const score =
        parseInt(document.getElementById(`${prefix}_${stat}_score`).value) ||
        10;
      const baseMod = Math.floor((score - 10) / 2);
      const isProficient = document.getElementById(
        `${prefix}_${stat}_save_prof`
      ).checked;
      const total = baseMod + (isProficient ? profBonus : 0);
      const totalDisplay = document.getElementById(
        `${prefix}_${stat}_save_total`
      );
      if (totalDisplay) {
  ...
  function applyHealthChange(amount) {
    ...
    if (actualChange !== 0) {
      ...
      broadcast("CMD_SHOW_DAMAGE_FLOAT", {
        amount: actualChange,
        tokenId: token.id,
      });
      broadcast("CMD_APPLY_TOKEN_ANIMATION", {
        tokenId: token.id,
        animationClass,
        timeout,
      });
  ...
  function updateAoeControls() {
    ...
    document.querySelectorAll(".aoe-params").forEach((paramDiv) => {
      paramDiv.style.display =
        paramDiv.id === `params-${activeAoeType}` ? "block" : "none";
    });
  }
  function broadcastAoeState() {
    ...
    const aoeData = {
      ...
      origin: {
        x:
          token.position.x +
          ((token.position.sizeMultiplier || 1) * cellSize) / 2,
        y:
          token.position.y +
          ((token.position.sizeMultiplier || 1) * cellSize) / 2,
      },
      params: { color: aoeColorInput.value },
    };
    switch (activeAoeType) {
      case "line":
        aoeData.params.width =
          parseInt(document.getElementById("aoeLineWidth").value) || 1;
        break;
      case "cone":
        aoeData.params.length =
          parseInt(document.getElementById("aoeConeLength").value) || 1;
        break;
      case "cube":
        aoeData.params.size =
          parseInt(document.getElementById("aoeCubeSize").value) || 1;
        break;
      case "sphere":
        aoeData.params.radius =
          parseInt(document.getElementById("aoeSphereRadius").value) || 1;
        break;
      case "cylinder":
        aoeData.params.radius =
          parseInt(document.getElementById("aoeCylinderRadius").value) || 1;
        break;
    }
    broadcast("CMD_DRAW_AOE", { aoeData });
  }
  ...
  function updateDoorList() {
    ...
    doors.forEach((door) => {
      ...
      li.innerHTML = `<span class="door-name" data-id="${
        door.id
      }" title="Haz clic para editar">${door.name}</span>
            <div class="door-actions"><button class="toggle-door-btn ${
              door.isOpen ? "open" : ""
            }" data-id="${door.id}">${
        door.isOpen ? "Cerrar" : "Abrir"
      }</button><button class="delete-door-btn" data-id="${
        door.id
      }" title="Eliminar Acceso">X</button></div>`;
      doorListUl.appendChild(li);
    });
    doorListUl
      .querySelectorAll(".toggle-door-btn")
      .forEach((btn) => btn.addEventListener("click", toggleDoorState));
    doorListUl
      .querySelectorAll(".delete-door-btn")
      .forEach((btn) => btn.addEventListener("click", deleteDoor));
    doorListUl
      .querySelectorAll(".door-name")
      .forEach((nameSpan) =>
        nameSpan.addEventListener("click", makeDoorNameEditable)
      );
  }
  ...
  function updateTokenList() {
    ...
    if (tokens.length === 0) {
      tokenListUl.innerHTML = `<p class="no-tokens-message">Aún no hay fichas en el tablero.</p>`;
      return;
    }
    const sortedTokens = [...tokens].sort(
      (a, b) => (b.stats.initiative || 0) - (a.stats.initiative || 0)
    );
    sortedTokens.forEach((token) => {
      const li = document.createElement("li");
      li.dataset.id = token.id;
      const typeIconHTML = `<span class="token-list-icon ${
        token.identity.type === "player"
          ? "icon-player-list"
          : "icon-enemy-list"
      }"></span>`;
      const borderStyle = token.appearance.borderColor
        ? `border: 3px solid ${token.appearance.borderColor};`
        : "none";
      const imageStyle = token.identity.image
        ? `background-image: url(${token.identity.image}); background-size: cover; background-position: center;`
        : `background-color: ${token.appearance.color};`;
      const previewContent = token.identity.image ? "" : token.identity.letter;
      li.innerHTML = ` <div class="token-list-preview" style="${imageStyle} ${borderStyle}">${previewContent}</div>
            <div class="token-list-header">${typeIconHTML}<span>${
        token.identity.name
      }</span></div>
            <div class="token-list-details"> <span>Iniciativa: ${
              token.stats.initiative || 0
            }</span> <span>♥️ ${token.stats.health.current}/${
        token.stats.health.max
      }</span> <span>👁️ ${
        token.stats.vision.radius
      }</span> </div> <button class="delete-token-btn" data-id="${
        token.id
      }" title="Eliminar Ficha">X</button>`;
      tokenListUl.appendChild(li);
    });
    tokenListUl.querySelectorAll(".delete-token-btn").forEach((btn) =>
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
    ...
    tokenListUl
      ...
      .forEach((li) =>
        li.addEventListener("click", (e) =>
          selectToken(parseInt(e.currentTarget.dataset.id))
        )
      );
    if (selectedTokenId) {
      const selectedLi = tokenListUl.querySelector(
        `li[data-id="${selectedTokenId}"]`
      );
      if (selectedLi) selectedLi.classList.add("selected-in-list");
    }
  }
    ...
    const newAbility = {
      id:
        editingAbilityIndex !== null
          ? token.info.abilities[editingAbilityIndex].id
          : Date.now(),
      name: name,
    ...
    token.info.inventory.forEach((item, index) => {
      ...
      card.innerHTML = `
                <div class="item-badges">
                    <span class="item-badge item-price">${
                      item.price || ""
                    }</span>
                    <span class="item-badge item-weight">${
                      item.weight || ""
                    }</span>
                </div>
      ...
      card
        .querySelector(".item-action-btn.edit")
      ...
      card
        .querySelector(".item-action-btn.delete")
  ...
  function renderTokenStatesEditor(token) {
    ...
    editTokenStatesList
      .querySelectorAll(".delete-state-btn")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
  ...
  tokenInfoEdit.querySelectorAll("input").forEach((input) => {
    input.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        ...
        const token = tokens.find((t) => t.id === selectedTokenId);
        if (token) updateTokenInUI(token);
        tokenInfoView.style.display = "block";
        tokenInfoEdit.style.display = "none";
      }
    });
  ...
  function getCharacterLibrary() {
    return JSON.parse(localStorage.getItem(CHAR_LIB_STORE)) || [];
  }
  async function saveCharacterToLibrary(characterData) {
    ...
    const existingIndex = library.findIndex(
      (char) => char.identity.name === cleanCharacter.identity.name
    );
    ...
    showNotification(
      `¡<strong style="color: var(--text-gold);">${cleanCharacter.identity.name}</strong> se ha guardado en la biblioteca!`
    );
  ...
  function renderCharacterLibrary() {
    ...
    savedCharactersGrid.innerHTML = "";
    const hasCharacters = library.length > 0;
    noSavedCharactersMessage.style.display = hasCharacters ? "none" : "block";
    ...
    library.forEach((char, index) => {
      ...
      const imageStyle = char.identity.image
        ? `background-image: url(${char.identity.image});`
        : `background-color: ${char.appearance.color};`;
      const previewContent = char.identity.image ? "" : char.identity.letter;
      card.innerHTML = ` <div class="saved-char-preview">
      ...
      card.addEventListener("click", () => {
        ...
        if (
          characterToAdd.info &&
          Array.isArray(characterToAdd.info.inventory)
        ) {
          characterToAdd.info.inventory =
            characterToAdd.info.inventory.map(migrateInventoryItem);
        }
        ...
        if (characterToAdd) {
          ...
          showNotification(
            `¡<strong style="color: var(--text-gold);">${characterToAdd.identity.name}</strong> ha regresado al tablero!`
          );
        }
      });
      card
        .querySelector(".delete-saved-char-btn")
        .addEventListener("click", (e) => {
          e.stopPropagation();
          if (
            confirm(
              `¿Eliminar a "${char.identity.name}" de la biblioteca? Esta acción no se puede deshacer.`
            )
          ) {
  ...
  async function createTokenFromForm() {
    ...
    const tokenData = {
      ...
      isDiscovered:
        document.querySelector('input[name="add_tokenType"]:checked').value ===
        "player",
      identity: {
        name,
        type: document.querySelector('input[name="add_tokenType"]:checked')
      ...
      stats: {
        ...
        proficiencyBonus:
          parseInt(document.getElementById("add_proficiency_bonus").value) || 2,
        speed: parseInt(document.getElementById("add_speed").value) || 30,
        armorClass:
          parseInt(document.getElementById("add_armor_class").value) || 10,
        characteristics,
    ...
    addTokenToBoard(tokenData);
    showNotification(
      `¡<strong style="color: var(--text-gold);">${tokenData.identity.name}</strong> se ha unido a la aventura!`
    );

    // 4. Limpiar el formulario para la siguiente entrada
    resetAddTokenForm();
  }
  function addTokenToBoard(tokenData) {
    tokens.push(tokenData);
    updateTokenList();
    broadcast("CMD_CREATE_TOKEN", { tokenData });
    if (visionModeActive) {
      broadcast("CMD_DRAW_VISION", { tokens, walls, cellSize });
    }
  }
  ...
  function loadDmNotes() {
    const savedNotes = localStorage.getItem(DM_NOTES_KEY);
    if (savedNotes) {
      dmNotesTextarea.value = savedNotes;
    }
  }
  dmNotesTextarea.addEventListener("input", () => {
    localStorage.setItem(DM_NOTES_KEY, dmNotesTextarea.value);
  });
  function filterList(inputElement, listContainer) {
    ...
    items.forEach((item) => {
      ...
      const description =
        ...
        "";
      const isVisible =
        title.includes(filterText) || description.includes(filterText);
  ...
  function filterInfoList(inputElement, listContainer) {
    ...
    items.forEach((item) => {
      ...
      const title =
        item.querySelector(".info-card-title")?.textContent.toLowerCase() || "";
      const description =
        item.querySelector(".info-card-desc")?.textContent.toLowerCase() || "";
      const isVisible =
        title.includes(filterText) || description.includes(filterText);
  ...
  searchAbilitiesInput.addEventListener("input", () =>
    filterInfoList(searchAbilitiesInput, editAbilitiesList)
  );
  searchInventoryInput.addEventListener("input", () => {
    filterList(searchInventoryInput, editInventoryList);
  });
  function handleExportScene() {
    ...
    if (!playerViewWindow || playerViewWindow.closed) {
      showCustomModal({
        title: "Atención",
        message:
          "La ventana del jugador debe estar abierta para exportar el estado completo (incluida la niebla de guerra)..",
        type: "warning",
      });

      return;
    }
    const mapImageDataUrl = document.getElementById("mapImageInput")._dataUrl;
    if (!mapImageDataUrl) {
      showCustomModal({
        title: "Error",
  ...
  function finalizeExport(sceneData) {
    ...
    a.href = url;
    a.download = "dm-arsenal-scene.json";
  ...
  function handleImportScene(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (
          !sceneData.mapImageDataUrl ||
          !sceneData.tokens ||
          !sceneData.walls ||
          !sceneData.gridSettings
        ) {
          throw new Error(
            "El archivo no parece ser una escena válida de DM Arsenal."
          );
        }
        isMapLoaded = true;
        document.getElementById("mapImageInput")._dataUrl =
          sceneData.mapImageDataUrl;
        const gs = sceneData.gridSettings;
        gridVisible = gs.visible;
        gridColor = gs.color;
        gridOpacity = gs.opacity;
        gridOffsetX = gs.offsetX;
        gridOffsetY = gs.offsetY;
        ...
        tokens = sceneData.tokens.map((tokenData) => {
          ...
          if (
            migratedToken.info &&
            Array.isArray(migratedToken.info.inventory)
          ) {
            migratedToken.info.inventory =
              migratedToken.info.inventory.map(migrateInventoryItem);
          }
  ...
  function handleExportLibrary() {
    ...
    if (library.length === 0) {
      showCustomModal({
        title: "Atención",
        message:
          "Tu biblioteca de personajes está vacía. No hay nada que exportar.",
        type: "warning",
      });
      return;
    }
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "dm-arsenal-character-library.json";
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (!Array.isArray(importedData)) {
          ...
          throw new Error(
            "El archivo no es una biblioteca válida (no es un array)."
          );
        }
        ...
        importedCharacters.forEach((character) => {
          if (character.info && Array.isArray(character.info.inventory)) {
            character.info.inventory =
              character.info.inventory.map(migrateInventoryItem);
          }
        });
        ...
        if (importedCharacters.length === 0) {
          await showCustomModal({
            ...
            message:
              "El archivo que intentas importar no contiene ningún personaje.",
            type: "warning",
          });
        ...
        if (importMode === true) {
          for (const newChar of importedCharacters) {
            if (!newChar.identity || !newChar.identity.name) {
              console.warn(
                "Se encontró un personaje sin identidad o nombre en el archivo importado. Será omitido."
              );
              continue;
            }
            const existingIndex = currentLibrary.findIndex(
              (char) => char.identity.name === newChar.identity.name
            );
            if (existingIndex > -1) {
              const doOverwrite = await showCustomModal({
          ...
          const replaceConfirmed = await showCustomModal({
            ...
            message:
              "¿Estás SEGURO de que quieres borrar tu biblioteca actual y reemplazarla? Esta acción no se puede deshacer.",
            type: "warning",
          ...
          if (replaceConfirmed) {
            ...
            await showCustomModal({
              ...
              message:
                "La importación ha sido cancelada. Tu biblioteca no ha sido modificada.",
              type: "info",
            });
  ...
  function setupPanelNavigation() {
    const panelColumnsWithNav = document.querySelectorAll(".panel-column");
    panelColumnsWithNav.forEach((column) => {
      const navButtons = column.querySelectorAll(".panel-nav-button");
      const views = column.querySelectorAll(".panel-view");
      navButtons.forEach((button) => {
        button.addEventListener("click", () => {
          const viewIdToShow = button.dataset.view;
          views.forEach((view) => view.classList.remove("active"));
          navButtons.forEach((btn) => btn.classList.remove("active"));
          const viewToShow = column.querySelector("#" + viewIdToShow);
          if (viewToShow) {
  ...
  function setupTokenEditorNavigation() {
    ...
    const navButtons = editor.querySelectorAll(".token-nav-button");
    const views = editor.querySelectorAll(".token-view-panel");
    navButtons.forEach((button) => {
      button.addEventListener("click", () => {
        const viewIdToShow = button.dataset.view;
        views.forEach((view) => view.classList.remove("active"));
        navButtons.forEach((btn) => btn.classList.remove("active"));
        const viewToShow = editor.querySelector("#" + viewIdToShow);
        if (viewToShow) {
          viewToShow.classList.add("active");
        }
        button.classList.add("active");
  ...
  function resetAddTokenForm() {
    ...
    document
      .querySelectorAll('#add_saving_throws_list input[type="checkbox"]')
  ...
  if (resetInitiativeBtn) {
    resetInitiativeBtn.addEventListener("click", async () => {
      ...
      const confirmed = await showCustomModal({
        ...
        message:
          "¿Estás seguro de que quieres establecer la iniciativa de TODAS las fichas en el tablero a 0?",
        type: "confirm",
  ...
  if (editTokenTurnInput) {
    const updateInitiative = () => {
      ...
      showNotification(
        `Iniciativa de ${token.identity.name} actualizada a ${newInitiative}.`
      );
    };
  ...
  if (toggleDefeatedBtn) {
    toggleDefeatedBtn.addEventListener("click", () => {
      ...
      toggleDefeatedBtn.textContent = token.isDefeated
        ? "Reanimar Enemigo"
        : "Abatir Enemigo";
  ...
  window.addEventListener("beforeunload", (event) => {
    ...
    const confirmationMessage =
      "Tienes cambios sin guardar en la escena. Si sales ahora, se perderá todo el progreso. ¿Estás seguro de que quieres salir?";
    ...
    const newItem = {
      id:
        editingItemIndex !== null
          ? token.info.inventory[editingItemIndex].id
          : Date.now(),
      name: name,
  ...
  cancelAbilityBtn.addEventListener("click", () =>
    abilityModal.classList.remove("open")
  );
  ...
  const addPresentationContainer = document.getElementById(
    "addPresentationContainer"
  );
  ...
  const toggleAddBossFields = () => {
    ...
    addPresentationContainer.style.display =
      isEnemy && isBoss ? "block" : "none";
  };
  ...
  editBossToggle.addEventListener("change", () => {
    editPresentationContainer.style.display = editBossToggle.checked
      ? "block"
      : "none";
